...
class TokenUpdateTransaction(Transaction):
    ...
    def __init__(
        ...
        self.token_key_verification_mode: TokenKeyValidation = (
    ...
    def set_treasury_account_id(
        self, treasury_account_id: AccountId
    ...
    def set_key_verification_mode(
        self, key_verification_mode: TokenKeyValidation
    ) -> "TokenUpdateTransaction":
        """
    ...
    def _build_proto_body(self) -> token_update_pb2.TokenUpdateTransactionBody:
        ...
        token_update_body = token_update_pb2.TokenUpdateTransactionBody(
            ...
            treasury=(
                self.treasury_account_id._to_proto()
                if self.treasury_account_id
                else None
            ),
    ...
    def _set_keys_to_proto(
        self, token_update_body: token_update_pb2.TokenUpdateTransactionBody
    ) -> None:
        """
        ...
        if self.freeze_key:
            token_update_body.freezeKey.CopyFrom(
                self.freeze_key.public_key()._to_proto()
            )
        if self.wipe_key:
            token_update_body.wipeKey.CopyFrom(self.wipe_key.public_key()._to_proto())
        if self.supply_key:
            token_update_body.supplyKey.CopyFrom(
                self.supply_key.public_key()._to_proto()
            )
        if self.metadata_key:
            token_update_body.metadata_key.CopyFrom(
                self.metadata_key.public_key()._to_proto()
            )
        if self.pause_key:
            token_update_body.pause_key.CopyFrom(
                self.pause_key.public_key()._to_proto()
            )
