...
class Endpoint:
    ...
    def __init__(
        self, address: bytes = None, port: int = None, domain_name: str = None
    ) -> None:
        """
    ...
    def _to_proto(self) -> ServiceEndpoint:
        ...
        return ServiceEndpoint(
            ipAddressV4=self._address, port=self._port, domain_name=self._domain_name
        )
    ...
    def from_dict(cls, json_data: EndpointDict) -> "Endpoint":
        ...
        """
        if (
            "ip_address_v4" not in json_data
            or "port" not in json_data
            or "domain_name" not in json_data
        ):
            raise ValueError(
