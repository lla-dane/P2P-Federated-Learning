...
class FileCreateTransaction(Transaction):
    ...
    def __init__(
        ...
        self.expiration_time: Optional[Timestamp] = (
            expiration_time
            if expiration_time
            else Timestamp(int(time.time()) + self.DEFAULT_EXPIRY_SECONDS, 0)
        )
    ...
    def _from_proto(
        self, proto: file_create_pb2.FileCreateTransactionBody
    ) -> "FileCreateTransaction":
        """
        ...
        """
        self.keys = (
            [PublicKey._from_proto(key) for key in proto.keys.keys]
            if proto.keys.keys
            else []
        )
