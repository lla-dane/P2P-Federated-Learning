...
class TransactionGetReceiptQuery(Query):
    ...
    def _make_request(self) -> query_pb2.Query:
        ...
        try:
            ...
            transaction_get_receipt = (
                transaction_get_receipt_pb2.TransactionGetReceiptQuery()
            )
            transaction_get_receipt.header.CopyFrom(query_header)
            transaction_get_receipt.transactionID.CopyFrom(
            ...
            if not hasattr(query, "transactionGetReceipt"):
                raise AttributeError(
                    "Query object has no attribute 'transactionGetReceipt'"
                )
    ...
    def _get_method(self, channel: _Channel) -> _Method:
        ...
        """
        return _Method(
            transaction_func=None, query_func=channel.crypto.getTransactionReceipts
        )
    ...
    def _map_status_error(
        ...
        return ReceiptStatusError(
            status,
            self.transaction_id,
            TransactionReceipt._from_proto(
                response.transactionGetReceipt.receipt, self.transaction_id
            ),
        )
    ...
    def execute(self, client: Client) -> TransactionReceipt:
        ...
        return TransactionReceipt._from_proto(
            response.transactionGetReceipt.receipt, self.transaction_id
        )
