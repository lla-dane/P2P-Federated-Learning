...
class _Channel:
    ...
    def smart_contract(self):
        ...
        if self._smart_contract is None and self.channel is not None:
            self._smart_contract = (
                smart_contract_service_pb2_grpc.SmartContractServiceStub(self.channel)
            )
    ...
    def address_book(self):
        ...
        if self._address_book is None and self.channel is not None:
            self._address_book = address_book_service_pb2_grpc.AddressBookServiceStub(
                self.channel
            )
